(function () {
  ...
  function setupClapDetection() {
    ...
    navigator.mediaDevices.getUserMedia({ audio: true }).then(function (stream) {
      ...
      function tick() {
        ...
        var now = Date.now();
        if (avg > 48 && now - lastPeak > 60) {
          lastPeak = now;
          peaks.push(now);
          peaks = peaks.filter(function (t) { return now - t < 600; });
          if (peaks.length >= 2) {
            var dt = peaks[peaks.length - 1] - peaks[peaks.length - 2];
            if (dt >= 80 && dt <= 550 && now - lastToggle > 1200) {
              lastToggle = now;
              peaks = [];
              flashClapCue();
              toggleFog("mic");
  ...
  if (form) {
    form.addEventListener("submit", function (e) {
      ...
      var payload = {
        name: name, email: email, phone: phone, neighborhood: neighborhood,
        _subject: "FogTap waitlist signup", _template: "table", _captcha: "false", _honey: ""
      };
      ...
      function tryPost(i) {
        ...
        fetch(endpoints[i], {
          ...
          if (pack.res.ok || pack.data.success === "true" || pack.data.success === true || pack.data.ok) {
